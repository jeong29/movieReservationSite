...
//		model.addAttribute("oneToOneResult", result3);
	...
	public String memberOneToOneList(MemberVO memberVO, Model model,OneToOneVO otoVO) {
	...
	//어드민이 회원에게 일대일문의 답변시 
	@RequestMapping(value = "/oneToOneUpdate.do")
	public String oneToOneUpdate(OneToOneVO otoVO,Model model) {
		//쿼리 등록하기
		int update = customerService.updateAnswer(otoVO);
		//OneToOneVO detail = customerService.selectOneToOneDetail(otoVO);
		model.addAttribute("result", update);
	//	model.addAttribute("page", "/jsp/admin/admin_member_oneToOne_result.jsp");
		
		return "/jsp/admin/admin_member_oneToOne_result.jsp";
	}
	
	//관리자 권한 부여시
	@ResponseBody
	@RequestMapping(value = "adminGrantAjax.do")
	public int adminGrantAjax(MemberVO memberVO){
		int list = memberService.updateAdminGrant(memberVO);
		return list;
	}
